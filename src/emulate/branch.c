 ...
 */
typedef enum { EQ = 0b0000, NE = 0b0001, GE = 0b1010, LT = 0b1011, GT = 0b1100, LE = 0b1101, AL = 0b1110 } CondType;

static bool pstateHandler(int64_t cond) {
    int cond_type = extractBits(cond, 0, 3);
    if (cond_type == EQ)
        return pstate.Z == 1;
    else if (cond_type == NE)
        return pstate.Z == 0;
    else if (cond_type == GE)
        return pstate.N == pstate.V;
    else if (cond_type == LT)
        return pstate.N != pstate.V;
    else if (cond_type == GT)
        return pstate.Z == 0 && pstate.N == pstate.V;
    else if (cond_type == LE)
        return !(pstate.Z == 0 && pstate.N == pstate.V);
    else if (cond_type == AL)
        return true;
    else
        return false;  // not a valid condition type
}
...
#define is_register(instruction) \
    (extractBits((instruction), 10, 31) == 0b1101011000011111000000 && extractBits((instruction), 0, 4) == 0)
#define is_conditional(instruction) \
    (extractBits((instruction), 24, 31) == 0b01010100 && extractBits((instruction), 4, 4) == 0)
...
bool branchHandler(INST inst) {
    ...
    if (is_unconditional(inst)) {
        ...
        if (pstateHandler(cond))
            programCounter += simm19 * 4;
        else
            programCounter += 4;
    } else {
